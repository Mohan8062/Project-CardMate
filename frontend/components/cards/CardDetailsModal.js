...
import { View, Text, TouchableOpacity, ScrollView, Modal, SafeAreaView, Linking, Platform } from 'react-native';
                    ...
                    {selectedCard && (
                        <View>
                            <View style={[styles.detailCardTop, { backgroundColor: theme.bg, borderColor: theme.border }]}>
                                ...
                                {selectedCard.designation && <Text style={[styles.detailSubText, { color: theme.textSecondary }]}>{selectedCard.designation}</Text>}
                                {selectedCard.company && <Text style={[styles.detailSubText, { fontWeight: '700', color: theme.textSecondary }]}>{selectedCard.company}</Text>}
                                ...
                                <View style={{ flexDirection: 'row', gap: 20, paddingHorizontal: 10 }}>
                                    ...
                                    <TouchableOpacity style={styles.quickActionBtn} onPress={() => {
                                        ...
                                        if (emailList.length > 0) {
                                            ...
                                            const body = encodeURIComponent(`Hi ${selectedCard.name},\n\nIt was great meeting you at ${selectedCard.event_name || 'the event'}. Let's keep in touch!\n\nBest,\n${user?.username || 'CardMate User'}`);
                                            Linking.openURL(`mailto:${emailList[0]}?subject=${subject}&body=${body}`);
                                        }
                                    }}>
                                        <View style={[styles.quickActionIcon, { backgroundColor: theme.accent, borderRadius: 30 }]}>
                            ...
                            <View style={[styles.detailInfoSection, { backgroundColor: theme.bg }]}>
                                ...
                                {selectedCard.notes && (
                                    <View style={[styles.detailItem, { borderBottomColor: theme.border }]}>
                                        ...
                                        <Text style={[styles.detailValue, { color: theme.textPrimary }]}>{selectedCard.notes}</Text>
                                    </View>
                                )}
                            </View>
